...
def generate_panel(face_w, face_h, edge_mode, slot_parity=0):
    ...
    edge_mode: dict with keys {"top","right","bottom","left"} each in {"TAB","SLOT"}
      TAB  => protrude outward on odd segments (matches your rectangles)
      SLOT => indent inward on slot_parity segments (so we can align squares)
    """
    half_w = face_w / 2.0
    half_h = face_h / 2.0
...
rect_pts_B = generate_panel(w, h, EDGE_MAP_RECT_FIX)

# ✅ KEY FIX: squares use slot_parity=1 so slots start on ODD segments (aligns to rect tabs)
