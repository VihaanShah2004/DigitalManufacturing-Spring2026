...
if add_art == "y":
    png_path = input("Enter path to PNG file (e.g. image.png): ").strip()
...
# ---- PNG artwork (raster engrave), centered, scaled to fit without leaving the panel ----
if png_data is not None and art_target is not None:
    ...
    # Preserve aspect ratio: scale to fit inside usable box
    ...
    # Clip so it NEVER spills outside the panel usable rectangle
    clip_id = f"clip_{art_target}"

    file.write('  <defs>\n')
    file.write(f'    <clipPath id="{clip_id}">\n')
    file.write(f'      <rect x="{(cx-usable_w/2):.3f}" y="{(cy-usable_h/2):.3f}" '
               f'width="{usable_w:.3f}" height="{usable_h:.3f}" />\n')
    file.write('    </clipPath>\n')
    file.write('  </defs>\n')

    # Put PNG in its own group (black layer conceptually; actual image is raster)
