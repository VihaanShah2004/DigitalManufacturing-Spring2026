...
# degrees to radians (no math library): multiply by pi/180
pi = 3.141592653589793
def deg_to_rad(deg):
    return deg * pi / 180

# sin and cos from a simple polynomial (Taylor) so we don't need the math library
def cos_deg(deg):
    r = deg_to_rad(deg)
...
# recursive branching: draw one segment from (px,py) at angle_deg, then two branches at angle +/- branch_angle.
def branch_segments(px, py, length, angle_deg, depth, branch_angle, length_scale, curvature=0):
    dx = length * cos_deg(angle_deg)
    dy = length * sin_deg(angle_deg)
    x2 = px + dx
    y2 = py + dy
    segs = [(px, py, x2, y2)]
    if depth <= 0:
