...
tab_width = min(w, h) / 6.2   # larger tabs (keep 6*tw < w so flat segments stay positive)
...
# bolt holes: 2-56 square nut clearance. Radius matches nut (width across corners ~6.5 mm) so nut fits in groove.
nut_hole_radius_mm = 3.35   # diameter 6.7 mm, clears 6.5 mm nut
circle_radius = nut_hole_radius_mm
# hole centers = center of each tab (just inside the tab). Square panel: top/right/bottom/left tabs.
hole_top = (w / 2, -mt / 2)       # top edge: middle tab center (tab sticks up y=-mt to 0)
hole_right = (w + mt / 2, w / 2) # right edge: tab sticks out x=w to w+mt
hole_bottom = (w / 2, w + mt / 2)
hole_left = (-mt / 2, w / 2)
